...
export const useStore = (props) => {
  ...
  useEffect(() => {
    ...
    messageListener.on('INSERT', (payload) => handleNewMessage(payload.record))
    messageListener.on('DELETE', (payload) => handleDeletedMessage(payload.old_record))
    messageListener.subscribe()
    // Listen for changes to our users
    const userListener = realtime.channel('realtime:public:users')
    userListener.on('*', (payload) => handleNewOrUpdatedUser(payload.record))
    userListener.subscribe()
    // Listen for new and deleted channels
    const channelListener = realtime.channel('realtime:public:channels')
    channelListener.on('INSERT', (payload) => handleNewChannel(payload.record))
    channelListener.on('DELETE', (payload) => handleDeletedChannel(payload.old_record))
