...
export const useStore = (props) => {
  ...
  useEffect(() => {
    ...
    const realtime = new RealtimeClient(
      ...
      { params: { apikey: process.env.NEXT_PUBLIC_SUPABASE_KEY } }
    )
    realtime.connect()
    const messageListener = realtime.channel('realtime:public:messages')
    messageListener.on('INSERT', (payload) => handleNewMessage(payload.new))
    messageListener.on('DELETE', (payload) => handleDeletedMessage(payload.old))
    messageListener.subscribe()
    // Listen for changes to our users
    const userListener = realtime.channel('realtime:public:users')
    userListener.on('*', (payload) => handleNewOrUpdatedUser(payload.new))
    userListener.subscribe()
    // Listen for new and deleted channels
    const channelListener = realtime.channel('realtime:public:channels')
    channelListener.on('INSERT', (payload) => handleNewChannel(payload.new))
    channelListener.on('DELETE', (payload) => handleDeletedChannel(payload.old))
